...
function findFirstSingleChar(str) {
  // throw new Error('Not implemented');
  return str.split('').reduce((result, value, index, arr) => {
    if (result === null) {
      if (arr.reduce((acc, valueInner) => (value === valueInner ? acc + 1 : acc), 0) === 1) {
        return value;
      }
    }
    return result;
...
function isCreditCardNumber(ccn) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    const curNumber = Number(arr[i]);
    if (i % 2) {
      sum += (((curNumber * 2) > 9) ? ((curNumber * 2) - 9) : (curNumber * 2));
    } else {
      sum += curNumber;
    }
  }
...
function getDigitalRoot(num) {
  ...
  let n = num;
  let i = 0;
  const sum = (x) => String(x).split('').reduce((acc, curval) => acc + Number(curval), 0);
  n = sum(n);
  while (n > 9 && i < 10) {
    i += 1;
    n = sum(n);
...
function isBracketsBalanced(str) {
  ...
  const regExp = /\[\]|\(\)|<>|\{\}/;
  let testStr = str;
  while (testStr.search(regExp) !== -1) {
    testStr = testStr.replace(regExp, '');
  }
  return testStr === '';
}
...
function toNaryString(num, n) {
  ...
  let tempNum = num;
  do {
    newDec = Math.floor(tempNum / n);
    const indexDst = tempNum - newDec * n;
    tempNum = newDec;
    out = `${indexDst}${out}`;
  } while (tempNum > 0);
  return out;
...
function evaluateTicTacToePosition(position) {
  // throw new Error('Not implemented');
  function isVerticalHorizontalWinner(i) {
  ...
  for (let i = 0; i < position.length; i += 1) {
    winner = isVerticalHorizontalWinner(i);
    if (winner !== undefined) {
      break;
    }
  }
